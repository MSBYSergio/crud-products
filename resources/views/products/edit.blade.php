...
<div class="w-full mx-auto mt-20 max-w-lg bg-white shadow-lg rounded-lg p-6">
    <h1 class="text-2xl font-bold text-gray-800 mb-6 flex items-center">
        <i class="fas fa-box-open text-blue-500 mr-3"></i> Actualizar Producto
    </h1>
    <form action="{{route('products.update',$product)}}" method="post" class="space-y-4" enctype="multipart/form-data">
        ...
        <div>
            ...
            <textarea
                ...
                rows="4"
                class="w-full border border-gray-300 rounded-lg px-4 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                placeholder="Describe el producto">{{@old('descripcion',$product -> descripcion)}}</textarea>
        ...
        <div>
            ...
            <select
                ...
                <option value="{{$item -> id}}" @selected(@old('category_id',$item -> id) == $product -> category_id)>{{$item -> nombre}}</option>
                @endforeach
            </select>
        ...
        <div>
            ...
            <div class="mb-4">
                <div class="mt-2">
                    <img id="preview" name="preview" src="{{Storage::url($product -> imagen)}}" class="h-32 w-32 rounded-xl border border-gray-300 object-cover" alt="">
